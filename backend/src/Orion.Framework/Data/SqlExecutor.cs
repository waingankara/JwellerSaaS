...
public sealed class SqlExecutor(IConnectionFactory connectionFactory) : ISqlExecutor
{
    ...
    public async Task ExecuteInTransactionAsync(Func<IDbConnection, IDbTransaction, CancellationToken, Task> operation, CancellationToken cancellationToken) { using var c=await connectionFactory.CreateOpenConnectionAsync(cancellationToken).ConfigureAwait(false); using var tx=c.BeginTransaction(); await operation(c, tx, cancellationToken).ConfigureAwait(false); tx.Commit(); }
}
